...
<html lang="en" dir="ltr">
  <head>
    ...
    <link href="<?=base_url()?>assets/css/selectr.min.css" rel="stylesheet" type="text/css"> 
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
    
    
    <script src="<?=base_url()?>assets/js/selectr.min.js" type="text/javascript"></script>
    ...
    <div class="page">
      <div class="page-main">
        <div class="header py-4">
          <div class="container">
            <div class="d-flex">
              <a class="header-brand" href="<?=base_url('teacher-dashboard')?>">
                ...
                <div class="dropdown">
                  <a href="#" class="nav-link pr-0 leading-none" data-toggle="dropdown">
                    ...
                    <span class="ml-2 d-none d-lg-block">
                      <span class="text-default"><?=$this->session->userdata('teacher_name')?></span>
                    </span>
                  </a>
            ...
            <div class="row align-items-center">
              <div class="col-lg order-lg-first">
                <ul class="nav nav-tabs border-0 flex-column flex-lg-row">
                  <li class="nav-item">
                    <a href="<?=base_url('teacher-dashboard')?>" class="nav-link"><i class="fe fe-home"></i> Dashboard</a>
                  </li>
                  <?php
                    $privileges = $this->session->userdata('priv');
// print_r($privileges);
// die();
                    $roles=$users=$tables=$courses=$institutes=$teachers=$batch=$students=$news=$events=$video=$course=$assignment=0;
                    foreach($privileges as $priv){
                      ...
                      if($priv == 'course') $course = 1;
                       if($priv == 'assignment') $assignment = 1;
                  ...
                  <?php
                   ...
                   <?php }
                     if($teachers){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('teachers')?>" class="nav-link"><i class="fa fa-users"></i> Teachers</a>
                      </li>
                     <?php }
                     if($batch){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('teacher-batch')?>" class="nav-link"><i class="fa fa-user"></i> Batch</a>
                      </li>
                     <?php }
                     if($students){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('teacher-students')?>" class="nav-link"><i class="fa fa-user"></i> Students</a>
                      </li>
                     <?php }
                     if($news){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('news')?>" class="nav-link"><i class="fa fa-file"></i> News</a>
                      </li>
                     <?php }
                     if($events){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('events')?>" class="nav-link"><i class="fa fa-file"></i> Events</a>
                      </li>
                     <?php }
                     if($video){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('video')?>" class="nav-link"><i class="fa fa-file"></i> video</a>
                      </li>
                     <?php }
                      ...
                      if($assignment){
                        ?>
                        <li class="nav-item">
                        <a href="<?=base_url('assignment')?>" class="nav-link"><i class="fa fa-file"></i> Assignment</a>
                      </li>
                     <?php }
