<div class="my-3 my-md-5">
  <div class="container">
    <div class="row">
          ...
          <div class="offset-lg-4 col-md-4 alert alert-danger text-center">
            <button type="button" class="close" data-dismiss="alert"></button>
            Error subject adding to favourite! Please contact tech support.
          </div>
          <?php }

            if($this->session->flashdata('paper_remove') == 1){
          ?>
          <div class="offset-lg-4 col-md-4 alert alert-success text-center">
            <button type="button" class="close" data-dismiss="alert"></button>
            Subject remove from favourite
          </div>
          <?php
            }
            elseif($this->session->flashdata('paper_remove') == 2){
          ?>
          <div class="offset-lg-4 col-md-4 alert alert-danger text-center">
            <button type="button" class="close" data-dismiss="alert"></button>
            Error subject remove from favourite! Please contact tech support.
          </div>
          <?php } ?>
      <div id="roles-table" class="col-12">
        ...
        <div class="page-header" style="margin:0;">
            <h1 class="page-title">
                <a href="<?=base_url('teacher-semester')?>"><i class="fa fa-chevron-left"> Back to Semester</i></a>
            </h1>
        </div>
                        ...
                        <?php
                            ...
                            ?>
                              <tr>
                                ...
                                <td><img src="<?=$data['icon']?>" height="50" width="50"></td>
                                <td><?=date('d-m-Y',$data['updated_on'])?></td>
                                <td><a href="<?=base_url('teacher-chapters').'?id='.$data['id']?>" class="btn btn-info btn-sm">View</a>
<a href="<?=base_url('assignment?subject='.$data['id'])?>" class="btn btn-info btn-sm"> Assignments</a>
                                </td>
                               <td>
                                 
                                <?php if($data['favourite_status'] == 1){?>
                                <a href="<?=base_url('remove-favourite').'/'.$data['id']?>" class="btn btn-info btn-sm">Unfavorite</a>
                              <?php }else{ ?>
                                <a href="<?=base_url('add-favourite').'/'.$data['id']?>" class="btn btn-info btn-sm">Favorite</a>
                              <?php }?>
                              </td>
                              </tr>
                            <?php
