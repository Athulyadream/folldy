...
-
	...
	public function principal_login_view()
	{
		$this->load->view('principal/principal_login');
	}

	public function principal_login(){
		$username = $this->input->post('username');
		$password = $this->input->post('password');
		$out = $this->PrincipalModel->check_login($username,$password);
		if($out['status']==1){
			$data = $out['data'];
			$role = $this->AdminModel->get_role($data['role_id']);
			$rights = explode(',',$role['role_rights']);
			...
			$this->session->set_userdata('edit',$edit);
			$this->session->set_userdata('delete',$delete);
			$this->session->set_userdata('principal_id',$data['principal_id']);
			$this->session->set_userdata('principal_name',$data['principal_name']);
			$this->session->set_userdata('institute_id',$data['institute_id']);
		}
		echo json_encode($out);
	}
	public function principal_logout(){
		$this->session->sess_destroy();
		redirect(base_url('principal'));
	}
	public function forgot_password_principal(){
		$username = $this->input->post('username');
		$data = $this->PrincipalModel->forgot_password($username);
		 if(!empty($data)){
            $chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

            $phone=$data['principal_phone'];
            $password = substr( str_shuffle( $chars ), 0, 12 );
            $user_where['principal_phone'] = $data['principal_phone'];
            $user_where['principal_status'] = 1;
            $update_data['principal_password'] = password_hash($password,PASSWORD_DEFAULT);

            $update=$this->PrincipalModel->update_password($user_where,$update_data);
            if($update){
            	$message = 'Your new password is '.$password.' . Mesoki';
                $this->load->library('sms');
                $resp = $this->sms->sendsms($phone,$message);
