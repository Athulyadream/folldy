...
class InstituteController extends CI_Controller{
	...
	public function add_semester(){
		...
		$semester = array(
				...
				'institute_id' => $this->session->userdata('institute_id'),
				'publish_status' => 0,
				'semester_start_date' => $start_date1,
				'semester_end_date' => $end_date1,
		);
...
///////////////////////////////2021 additional athulya///////////////////////////////////////////////
	...
	public function principal_filter(){
		...
		$this->session->set_userdata('principal_filter',$filter);
		redirect(base_url('listprincipal'));
	}
	public function principal(){
		$filter = $this->session->userdata('principal_filter');
		$filter_data = array(
					'name'=> isset($filter['name'])? $filter['name']:'',
					'email'=> isset($filter['email'])? $filter['email']:'',
					'phone'=> isset($filter['phone'])? $filter['phone']:'',
		...
		$config['base_url'] = base_url('listprincipal');
		$config['total_rows'] = $this->InstituteModel->count_principal($filter_data);
		$config['per_page'] = $limit = 10;
		$config['prev_tag_open'] = '<li>';
		$config['prev_tag_close'] = '</li>';
		$config['num_tag_open'] = '<li>';
		...
		$offset = $this->uri->segment(2);
		$body_data = array(
							'principal'=>$this->InstituteModel->list_principal($limit,$offset,$filter_data),
							'subject'=>$this->InstituteModel->list_subjects(),
							'links'=>$links,
							'offset'=>$offset,
							'filter_data' => $filter_data,
		...
		$this->load->view('institute/header');
		$this->load->view('institute/principal',$body_data);
		$this->load->view('institute/footer');
	}
	public function add_principal(){
		...
		$data = array(
					...
					'principal_designation'=>$this->input->post('principal_designation'),

					'principal_password'=>password_hash($this->input->post('principal_password'),PASSWORD_DEFAULT),
					'principal_status'=>1,
					'created_on'=>time(),
					'updated_on'=>time(),
					'updated_by'=>$this->session->userdata('ins_user_id'),
		...
		$teacher_id = $this->InstituteModel->add_data($data,'principal');
		if($teacher_id){
	...
	public function edit_principal(){
		...
		$data = array(
					...
					'principal_phone'=>$this->input->post('principal_phone'),
					'principal_email'=>$this->input->post('principal_email'),
					'principal_designation'=>$this->input->post('edit_principal_designation'),
		...
		if($this->InstituteModel->edit_principal($data,$id)){
