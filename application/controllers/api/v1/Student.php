            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    if(!empty($student)){ 
                        ...
                        $out = array('status'=>0,'message'=>'There is no consultant',''=>new stdClass);
                    }
                }
            }
                ...
                if($check['status']== 0){
                    ...
                    if(!empty($teachers)){
                        foreach ($teachers as $key => $value) {
                            ...
                            $chapter1 = $this->Api_student->get_chapter_list($subject,$tid);
...
/////////////////////////////////principal athulya///////////////////////////////////////////
  function login_principal_post(){
        ...
        if (!$this->post('phone') && json_last_error() != 0)
        {
            ...
            if($phone && $password){
                $login = $this->Api_student->getDatas_login_principal($phone,$password);
                ///////////athulya///////////////////////////////
                // print_r($login);
                // die();
                if(!empty($login)){
                    $data_update=array('platform' => $platform);
                        ...
                        $this->Api_student->update_data_by_phone_principal($data_update,$phone);
                        if($login['session_value'] == 0){
                            $session['session_value'] = time().rand(0,10000);
                            if($fcm){
                                $session['fcm'] = $fcm;
                            }
                            $updte_where['principal_id'] = $login['principal_id'];
                            $this->Api_student->update_principal_details($session,$login['principal_id']);
                           
                        }
                        $login = $this->Api_student->getDatas_principal($login['principal_id']);
    ...
    function logout_principal_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            //if($consultant){
                $logout = $this->Api_student->logout_principal($user_id);
                if($logout){ 
                    $out = array('status'=>1,'message'=>'You are successfully logged out','data'=>'');
                    $this->Api_student->insert_log("APP - Log out",$user_id,'Student' );
    ...
    function change_password_principal_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if($check['status']== 0){
            ...
            $out = array('status'=>3,'message'=>'Session does not match.');
                ...
                $user['password'] = password_hash($new_password,PASSWORD_DEFAULT);
                $user['session_value'] = time().rand(0,10000);
                $id_return = $this->Api_student->update_principal_details($user,$id);
                if($id_return == 1){
                    $user_data = $this->Api_student->get_student_details($id);
    ...
    function forgot_password_principal_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        $active_user = $this->Api_student->get_principal_by_phone($phone);
        if(!empty($active_user)){
           
                $chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

                    $password = substr( str_shuffle( $chars ), 0, 8 );
                    $user_where['principal_phone'] = $phone;
                    // $user_where['teacher_status'] = 1;
                    $update_data['principal_password'] = password_hash($password,PASSWORD_DEFAULT);

                    $this->Api_student->update_principal($user_where,$update_data);
                        
                    $userdata = $this->Api_student->get_principal_by_phone($phone);
                    $phone_user = $userdata['principal_phone'];
