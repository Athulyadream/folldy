...
//////////////////***************** New updations-  athulya************///////////
  ...
  /////////////***************** New updations-  athulya************///////////
                    ...
                    if(!empty($student)){ 
                        $out = array('status'=>1,'message'=>'','todays'=>$todays,'subject' => $subject,'news'=>$news,'events' => $events,'recent_presentations' => $recentpresentation,'recent_chapter' => $recentchapter, 'notification_count'=> $nocount);
                    }
                    else{
                        $out = array('status'=>0,'message'=>'There is no consultant','data'=>new stdClass);
                    }
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    if($teacher != ''){
                        ...
                        $part = $this->Api_student->get_chapter_details_webview($chapter,$teacher);
                    if(!empty($part)){ 
                ...
                if($check['status']== 0){
                    ...
                    if($pid){ 
                        ...
                        // $this->M_log->insert_log("question_added", "student question added", $id);
                            ...
                            $title = "A new question asked from ".$sub['title'];
       ...
       if (!$this->post('id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $part = $this->Api_student->get_chapter_details_view($chapter_details['chapter_id'],$id);
                    if(!empty($part)){ 
                        $out = array('status'=>1,'message'=>'','background_image'=>$backgroundimage,'data'=>$part);
                    }
                    else{
        ...
        if (!$this->post('id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    // $publish_status = $this->Api_student->check_content_publish($presentation_id,$id);
                    $user_topic_id = $this->TeacherModel->get_user_topic($topic_parent_id,$id);
                    ...
                    if($user_topic_id == false){
                        ...
                        $publish_status = 1;
                        $teacher_topic_id = $this->TeacherModel->get_user_topic($presentation_id,$id);
                        // $presentation = $this->Api_student->get_childs($teacher_topic_id);
                        // $presentation_flow = $this->Api_student->get_childs_flow($teacher_topic_id);
                    }
            ...
            if($id){
                ...
                if($check['status']== 0){
                  ...
                  //  $teacher = $this->Api_student->get_subject_teacher($subject,$id,$batch_id);
                    ...
                    //echo $teacher;
                    if($teacher_id != ''){
                        $chapter = $this->Api_student->get_chapter_list_teacher($subject,$teacher_id);
                    foreach ($chapter as $key => $value) {
        ...
        if (!$this->post('teacher_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $data2 = $this->Api_student->get_chapter_details_teacher($chapter_id,$id);
//print_r($data);exit();
            ...
            if($id){
                ...
                if($check['status']== 0){
                    $out = array('status'=>3,'message'=>'Session does not match.');
                         ...
                         $check = $this->Api_student->check_publish($id,$chapter_id);
                            if(!empty($check)){
                                $data = array(
                                    'publish_status' => 1,
                                    'updated_on' => time(),
                    ...
                    if($result){ 
                        ...
                        $students = $this->Api_student->get_course_students_subject($subject);
  ...
  public function publish_chapter_content_post(){
        ...
        if (!$this->post('teacher_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $user_topic_id = $this->Api_student->get_user_topic($type_id,$id);
            
                    if($user_topic_id == false){
                        $this->clone_presentation($type_id,$id);
                    }
                }
    ...
    public function clone_presentation($id,$uid){
...
// die();
    ...
    public function draft_presentation($id,$uid){
         // $id = $this->input->post('id');
         // $tid = $this->input->post('tid');
                $this->load->model('PresentationModel');
                $this->load->model('TeacherModel');
            ...
            if($id && $type){
                ...
                if($check['status']== 0){
                    ...
                    if($pid){ 
                        ...
                        $tokens = implode(",", $toks);
                         $this->load->library('Fcm');
                            $title = "Teacher answered to a question in ".$sub['title'];
            ...
            if($check['status']== 0){
               ...
               if($c ==1){
                    ...
                    if($type =="teacher"){
                        ...
                        $publish_status = $this->Api_student->check_content_publish($practice_id,$id);

                    }else{
                        $publish_status  = 1;
                    }
             
                    $audios = $this->Api_student->practice_audio($practice_id,$id,$type);
                    if($audios){ 
        ...
        if (!$this->post('phone') && json_last_error() != 0)
        {
            ...
            elseif($check_student_email > 0){
                ...
                if($ins_data){
                    ...
                    $this->load->library('sms');
                    $msg = "Folldy register verification OTP is ". $otp;
                ...
                if($endTime < date('Y-m-d H:i:s')){
                    ...
                    if(!empty($token_data)){
                        foreach ($token_data as $key => $value) {
                            ...
                            $datan['content_id'] = $check['batch_id'];
                            $datan['created_by'] =0;
                            $datan['created_on'] = time();;
                ...
                if($check['status']== 0){
                    ...
                    if($up_data){
                        $data = array(
                            ...
                            $this->load->library('sms');
                            $msg = "Hi ".$invite_data['name'].", teacher verified your account. Welcome to Folldy.";
...
public function draft_presentation_post(){
         $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $this->draft_presentation($slide,$id);
                    $user_topic_id = $this->Api_student->get_user_topic($slide,$id);
                    $out = array('status'=>1,'message'=>'Presentation drafted successfully ','data'=>$user_topic_id);
                           $this->Api_student->insert_log("APP - Presentation draft".$slide,$id,'teacher' );
                        // $this->M_log->insert_log("question_added", "student question added", $id);
