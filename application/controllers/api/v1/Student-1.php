...
//print_r($data);exit();
    ...
    public function publish_chapter_post(){
        ...
        ////////////////////// Additional 2021 athulya open////////////////////////////
                        ...
                        $batch = $this->Api_student->get_semester_btach($semester);
                        ...
                        if (move_uploaded_file($_FILES["publish_audio"]["tmp_name"], $target_file)) {
                                ...
                                $teachername = $this->Api_student->getProfileDetails_teacher($id);
                                $datan = array(
                                        'batch_id' => $batch,
                                        'title'=> $title,
                                        'message' => $message, 
                    ...
                    if($type == 2){
                        ...
                        if($user_topic_id == 0 || $user_topic_id == "" ){
                            $this->clone_presentation($type_id,$id,$semester,$subject);
                             ...
                             $pid=  $this->Api_student->get_user_topic($type_id,$id,$semester,$subject);
                        }
                        $pname = $this->Api_student->get_topic_main_parent_name($type_id);
                    }
                    ...
                    if($pid){ 
                        ...
                        $tokens = implode(",", $toks);
                         $this->load->library('Fcm');
                            ...
                            $nid = $this->Api_student->add_data($datan,'notifications');
                             //Athulya Folldy Additional 2021 open
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    if($type == "presentation"){
                        ...
                        $out = array('status'=>1,'message'=>'Practice drafted successfully ','data'=>$prac_id);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($check['status']== 0){
               ...
               if($c ==1){
                    $assignment = array();
                    if($type=="student"){
                        $uid = $this->Api_student->get_subject_teacher($subject,$id);
                    }else if($type=="teacher"){
                        $uid = $id;
                    }
                    // echo $uid." - ".$type;
                    $assignment['pending'] = $this->Api_student->list_assignment($semester,$subject,$uid);
                    $assignment['closed'] = $this->Api_student->list_closed_assignment($semester,$subject,$uid);
...
// die();
                        ...
                        // }
                            ...
                            $batch = $this->Api_student->get_semester_btach($semester);
                                ...
                                $message = " A new assignment published in ".$chaptername;
                                $teachername = $this->Api_student->getProfileDetails_teacher($id);
                                $datan = array(
                                        'batch_id' => $batch,
                                        'title'=> $title,
...
// submit assignment file
    ...
    public function close_assignment_post(){
      ...
      public function list_assignment_students_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $assignment_id){
             $check = $this->Api_student->check_session_teacher($id,$session);
                ...
                $data_ass = $this->Api_student->get_assignment($assignment_id);
                // print_r($data_ass);
                // die();
                ...
                $semester = $data_ass['semester'];
                    ...
                    if($students){
                        ...
                        $out = array('status'=>1,'message'=>'','publish_status'=>$marks_publish,'data'=>$students);
                           $this->Api_student->insert_log("APP - list students".$assignment_id,$id,'teacher' );
                    }
                    else{
        ...
        if (!$this->post('student_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    //echo $teacher;
                    $wall = $this->Api_student->get_wall($batch);
                    if(!empty($wall)){ 
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    //echo $teacher;
                    $wall = $this->Api_student->get_wall_teacher($id);
                    if(!empty($wall)){ 
                        $out = array('status'=>1,'message'=>'','data'=>$wall);
                    }
                    else{
...
// die();
            ...
            ///////////////////////////////////////////////////////////////////////
                    ...
                    if(!empty($student)){ 
                        $out = array('status'=>1,'message'=>'','todays'=>$todays, 'notification_count'=> $nocount,'semesters'=>$semester,'profile'=>$student);
                    }
                    else{
                        $out = array('status'=>0,'message'=>'There is no consultant','data'=>new stdClass);
                    }
