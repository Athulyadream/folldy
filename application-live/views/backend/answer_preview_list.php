...
if(!empty($qt_table)){
  if(sizeof($qt_table)>1){
                          ...
                          foreach ($qt_table as $key => $qtdiv) {
                              ...
                              if(!empty($tab_q['qt_vals'])||!empty($tab_q['transactions'])){
                  ...
                  $.each(text.split('\r\n'), function(i2, v2){
                      $.each(v2.split('\t'), function(i3, v3){
                          ...
                          console.log($this.closest('table').find('.tr-quest:eq('+row+') td:eq('+col+')').html());
                           ...
                           if($this.closest('table').find('.ans-row:eq('+row+') td:eq('+col+') input').hasClass('number')){

                             // v3.replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                             // alert(v3)
                           val=v3.toString().replace(/,/g, "").replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                          // alert(v3);
                           $this.closest('table').find('.ans-row:eq('+row+') td:eq('+col+') input').val(val);
