    	...
    	<div class="row">
            ...
            elseif($this->session->flashdata('admin_created') == 2){
          	?>
          		<div class="offset-lg-4 col-md-4 alert alert-danger text-center">
            		<button type="button" class="close" data-dismiss="alert"></button>
           	 		Error adding Time Table! Please contact tech support.
          		</div>
          	<?php
            }
            ...
            elseif($this->session->flashdata('admin_updated') == 2){
          	?>
          		<div class="offset-lg-4 col-md-4 alert alert-danger text-center">
            		<button type="button" class="close" data-dismiss="alert"></button>
            		Error updating Time Table! Please contact tech support.
          		</div>
          	<?php } elseif($this->session->flashdata('admin_updated') == 2){
          	?>
          		<div class="offset-lg-4 col-md-4 alert alert-danger text-center">
            		<button type="button" class="close" data-dismiss="alert"></button>
            		Error adding Time Table! Please Select teachers for time table.
          		</div>
          	<?php } ?>
              			...
              			<tbody>
                			...
                			if(!empty($time_table)){
                						 ...
                						 for($i=1;$i<=$batch['batch_periods'];$i++){
		                  				?>
			                				<td>
			                					...
			                					<input type="text" class="form-control" name="sub_<?= $data['weekdays'] ?>_<?= $i ?>" id="sub_<?= $data['weekdays'] ?>_<?= $i ?>" placeholder="" maxlength="5" required><br>
                                <select class="form-control" name="teacher_<?= $data['weekdays'] ?>_<?= $i ?>" id="teacher_<?= $data['weekdays'] ?>_<?= $i ?>" required="required">
                                  <option selected="selected" disabled selected>Select</option>
                                  <?php 
                                  if(!empty($teachers)){
                                    foreach ($teachers as $key => $value) {
                                      echo '<option value="'.$value['teacher_id'].'" style="text-transform:capitalize">'.$value['teacher_name'].'</option>';
                                    }
                                  }
                                  ?>
                                </select>
                						...
                						for($i=1;$i<=$batch['batch_periods'];$i++){ 
                							...
                							if($time['period'] == $i && $time['weekdays'] == $data['weekdays']){ ?>
                								<td>
                								<input type="text" class="form-control" name="sub_<?= $data['weekdays'] ?>_<?= $i ?>" id="sub_<?= $data['weekdays'] ?>_<?= $i ?>" value="<?= $time['subject'] ?>" maxlength="5" required><br>
                                ...
                                <select class="form-control" name="teacher_<?= $data['weekdays'] ?>_<?= $i ?>" id="teacher_<?= $data['weekdays'] ?>_<?= $i ?>" required="required">
                                  ...
                                  if(!empty($teachers)){
                                    foreach ($teachers as $key => $value) {
                                      if($time['teacher_id'] == $value['teacher_id']){
                                        echo '<option value="'.$value['teacher_id'].'" style="text-transform:capitalize" selected>'.$value['teacher_name'].'</option>';
                                      }else{
                                        echo '<option value="'.$value['teacher_id'].'" style="text-transform:capitalize" >'.$value['teacher_name'].'</option>';
                                      }
