...
// print_r($data);
    ...
    function removeFromString1($str, $item) {
    $parts = explode(',', $str);
    // print_r($parts);
    //     exit();
    ...
    public function delete_trasaction($id,$qid){

        
        $gtedata=$this->get_transaction_by_id($id);
        //print(arg)
        $order=$gtedata['order_id'];



       $checkorder=$this->check_transaction_table($order,$qid);
       $transaction_id1=$checkorder['transaction_id'];

          if($transaction_id1!=0){
            $new_string=$this->removeFromString1($transaction_id1,$id);
        }else{
            $new_string=$this->removeFromString1($transaction_id1,$id);
        }
        ...
        $data=array('transaction_id'=>$new_string);
        ...
        if($this->db->delete('transactions')){
         ...
         if($this->db->update('q_table',$data)){
             ...
             $checkorder_last=$this->check_transaction_table($order,$qid);
        $transaction_id1=$checkorder['transaction_id'];
        $qt_id=$checkorder['qt_id'];
        $text_id=$checkorder['text_id'];
         //print_r($checkorder_last);
     //print_r($text_id);print_r($qt_id);
        //var_dump($transaction_id1);
        if($text_id==0&&$qt_id==0&&$new_string==0){
            // var_dump($transaction_id1);
            // print_r($transaction_id1);

            $this->db->where('question_id',$qid);
              $this->db->where('order_id',$order);
            $this->db->delete('q_table');
        }
...
//print_r(expression)
    ...
    public function get_transaction_by_id($id){
        $this->db->select('transactions.transaction_id,transaction_title,transaction_helptext,transaction_image,GROUP_CONCAT(keyword_value SEPARATOR ";") as keywords, GROUP_CONCAT(keyword_id ) as keyword_ids,transaction_image,order_id');
