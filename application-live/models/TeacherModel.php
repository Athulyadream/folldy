...
class TeacherModel extends CI_Model {
    ...
    public function list_papers($id){
        ...
        if($this->session->userdata('teacher_id') == '219'){
            print_r($ar);
        }
        $result = array();
        if(!empty($ar)){
           $this->db->select('data_id as id,data_name as name,paper_icon as icon,data.updated_on');
        ...
        // $this->db->where('teacher_id',$this->session->userdata('teacher_id'));
            ...
            foreach ($result as $key => $value) {
                ...
                $this->db->where('t.batch_id',$this->session->userdata('batch_id'));
    ...
    public function list_chapters($id,$limit,$offset){
        ...
        // $this->db->join('chapter_publish','chapter_publish.chapter_id = data.data_id AND chapter_publish.teacher_id='.$teacher,'left');
         ...
         $this->db->join('chapter_publish','chapter_publish.chapter_id = data.data_id  AND chapter_publish.batch_id ='.$this->session->userdata('batch_id') ,'left');
    ...
    // public function list_content_old($id,$limit,$offset){
        
    //     //march 6 edit athulya 2021
    ...
    public function list_content($id,$limit,$offset){
        ...
        $this->db->select('chapter_design.id, chapter_design.chapter_id, `type`, `type_id`, `status`, chapter_design.created_on, chapter_design.updated_on, chapter_design.updated_by, `presentation`, `topic_title`, `topic_color`, `topic_backgroundcolor`, `topic_backgroundimage`, `topic_type`, `topic_status`, `topic_bordercolor`, `topic_fontsize`, `topic_position_left`, `topic_position_top`, `topic_width`, `topic_height`, `topic_content_position_left`, `topic_content_position_top`,admin_name');
        $this->db->where('chapter_design.chapter_id',$id);
        $this->db->where('chapter_design.status >',0);
        // $this->db->where('teacher_id',$this->session->userdata('teacher_id'));
        // $this->db->where("(teacher_id=".$this->session->userdata('admin_id')." OR (teacher_id='NULL'))");
        ...
        foreach ($result as $value) {
            ...
            $this->db->where('chapter_content_id',$value['id']);
            $this->db->where('batch_id',$this->session->userdata('batch_id'));
            $queryP = $this->db->get('chapter_content_publish');
            $resultP = $queryP->row_array();
            // if($this->session->userdata('teacher_id') =='219'){
            //     print_r($this->db->last_query());
            //     print_r($resultP['publish_status']);
            //     die();
            // }
            $result[$x]['publish_status'] = $resultP['publish_status'];
            $result[$x]['teacher_id'] = $resultP['teacher_id'];
        ...
        if(!empty($subject)){
            foreach($subject as $key => $row){
                ...
                // $this->db->join('chapter_publish','chapter_publish.chapter_id = data.data_id AND chapter_publish.teacher_id='.$teacher,'left');
                   ...
                   $this->db->join('chapter_publish','chapter_publish.chapter_id = data.data_id  AND chapter_publish.batch_id ='.$row['batch_id'] ,'left');
...
/////athulya new 2021/////
    ...
    public function subject_chapter_dataid($id){
        $this->db->select('*');
        $this->db->where('data_id',$id);
        $this->db->where('data_status',1);
        return $this->db->get('data')->result_array();
    ...
    public function sem_subject_batch($btid,$subject){
     ...
     ////////////////////publish additional athulya 2021///////////////////////////////
    public function checkPublish_content($id,$batch){
        $this->db->where('chapter_content_id',$id);
        $this->db->where('batch_id',$batch);
        ...
        // return $this->db->count_all_results('chapter_content_publish');
         ...
         //$this->db->or_where('admin_email',$username);
        $data = $this->db->get('chapter_content_publish')->row_array();
    ...
    public function delete_wall($data){
     ...
     public function update_content_publish($data,$id){
        $this->db->where('id',$id);
        if($this->db->update('chapter_content_publish',$data)) return true;
        else return false;
    }
    ...
    ///  Additional 2021 Athulya
     ...
     function insert_log($action,$user_id='',$user=0) {
       ...
       public function list_assignment($limit,$offset,$filter){
        ...
        $this->db->where('semester',$semester);
        $this->db->where('subject',$subject);
        $this->db->where('created_by',$this->session->userdata('teacher_id'));
        // $this->db->limit($limit,$offset);
    ...
    public function count_assignment($filter){
        ...
        $this->db->where('semester',$semester);
        $this->db->where('subject',$subject);
        $this->db->where('updated_by',$this->session->userdata('teacher_id'));
        $this->db->where('assignment_status >',0);
...
public function get_semester_data($semester){
    ...
    public function list_content_plan($id,$limit,$offset){
        ...
        $this->db->where('semester_plan.teacher',$this->session->userdata('teacher_id'));
