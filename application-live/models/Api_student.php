...
///athulya change close////
    ...
    //Old buid usage no need current app start with chapter content partid as typeid-  and for presentation data for current/////////////////////
...
// }
    ...
    //Old buid usage no need current app start/////////////////////
    ...
    public function get_chapter_details_view($chapter,$teacher,$semester){
       ...
       $batch = $this->Api_student->get_semester_batch($semester);
        ...
        foreach($result as $key => $value){
            ...
            if($type =='1'){
                    $results[$key]['type']='video';
                //video
                    $this->db->select('video_id,video_title,video_link');
                    $this->db->where('video_status',1);
                    $this->db->where('type','1');
                    $this->db->where('video_id',$typeid);
                    $videolink = $this->db->get('video')->row_array();
                   

                $results[$key]['link'] = base_url().'uploads/video/'.$videolink['video_link'];
            }
            else if($type =='2'){
                //presentation
...
$batch = $this->Api_student->get_semester_batch($semester);
 ...
 $subject = $this->Api_student->get_chapter_subject($chapter);
            ...
            if($type =='1'){
                // $results[$key]['content_name']='video';
                    $results[$key]['type']='video';
                //video
                    $this->db->select('video_id,video_title,video_link');
                    $this->db->where('video_status',1);
    ...
    public function teacherSubjects($id){
        ...
        foreach ($result as $key => $value) {
            $this->db->select('data_id,data_name, data.updated_on, admin_name,data.updated_by ,chapter_publish.publish_status');
                ...
                $batchdet = $this->get_batch($value['batch_id']);
                $result[$key]['batch_name'] = $batchdet['batch_name'];
        }
        return $result;
    }
...
return $topics;
    ...
    public function get_student_count($batch){
                // $batches = explode(' OR ', $batch);
                $this->db->where('status',1);
                // $this->db->where("(batch_id)");
                $this->db->where('batch_id',$batch);
...
// die();
    ...
    public function get_chapter_details_library($chapter,$teacher){
        $this->db->select('chapter_design.id as part_id,chapter_design.type_id,chapter_design.type,chapter_design.topic_title ,chapter_design.chapter_id,video.thumb_nail,chapter_design.image');
    ...
    public function list_assignment_teacher($id,$batch,$subject){
        ...
        $result = $this->db->get('assignment')->result_array();
        foreach ($result as $key => $value) {
        	$assignment_student_view_percentage = $this->student_assignment_view_percentage($value['assignment_id']);
        	$result[$key]['percentage'] = $assignment_student_view_percentage;
            $result[$key]['score'] = 5;

            $assignment_files_question = array();
                $assignment_files_q = $this->assignment_files_question($value['assignment_id']);
                foreach ($assignment_files_q as $key1 => $value1) {
                    $assignment_files_question[] = base_url().'uploads/assignment_question/'.$value1['file'];
                }
                $result[$key]['assignment_files_question'] = $assignment_files_question;
        }
    // echo $this->db->last_query();
        return $result;
    ...
    public function get_assignment_view_student_teacher($asid,$sid){
        ...
        $result  =  $this->db->get('assignment')->row_array();
                ...
                $mark = $marks['marks'] ? $marks['marks'] : 0;
                $result['score'] = $mark;
                $result['marks_list'] = array('0','1','2','3','4','5');
                if($mark == 0){
                    $reject = 0;
                }else{
                    $reject = 1;
                }
                $result['reject_status'] = $reject;
    ...
    public function update_expiry_date($data,$id){
     ...
     public function check_content_count($where){
        $this->db->where($where);
        $query = $this->db->count_all_results('semester_plan');
        //echo $this->db->last_query();
        return $query;
    }
     public function check_chapter_count($where){
        $this->db->where($where);
        $query = $this->db->count_all_results('chapter_plan');
        return $query;
    }
    ...
    public function semester_plan_chapter($semester,$subject,$teacher){
        ...
        $this->db->select('data_id,data_name,data.updated_on,admin_name,data.updated_by,start_date,end_date as standard_date,chapter_plan.id as planid,chapter_plan_status,completed_date as actual_date ');
        $this->db->join('admins','admins.admin_id=data.updated_by ','left');
        // $this->db->join('chapter_publish','chapter_publish.chapter_id = data.data_id AND chapter_publish.teacher_id='.$teacher,'left');
        $this->db->join('chapter_plan','chapter_plan.chapter = data.data_id','left');
        $this->db->where('data_status',1);
        // $this->db->where('chapter_plan_status !=',3);

        $this->db->where("(chapter_plan_status is NULL OR chapter_plan_status = 1 OR chapter_plan_status = 2)");
        $this->db->where('data_type',2);
        $this->db->where('data_parent',$subject);
        ...
        foreach ($result as $key => $value) {
        	if($value['chapter_plan_status'] == 0){
        		$planstatus = 'Not approved';
        	}else if($value['chapter_plan_status'] == 1){
        		$planstatus = 'Approved';

        	}else if($value['chapter_plan_status'] == 2){
        		$planstatus = 'Completed';

        	}else if($value['chapter_plan_status'] == 3){
        		$planstatus = 'Deleted';

        	}
        	$result[$key]['status'] = $planstatus ;
        }
        return $result;
    }
     ...
     public function semester_plan_content($chapter,$id,$semester){
        ...
        $batch = $this->Api_student->get_semester_batch($semester);
        $this->db->select('semester_plan.id as content_planid,chapter_design.id as content_id, chapter_design.chapter_id, `type`, `type_id`, `status`, `topic_title`,content_start_date,content_end_date,content_completed_date,content_plan_status');
        $this->db->where('chapter_design.chapter_id',$chapter);
        $this->db->where('chapter_design.status >',0);
        $this->db->where("( semester_plan.content_plan_status = 1 OR semester_plan.content_plan_status = 2  OR semester_plan.content_plan_status is NULL)");
        ...
        $this->db->select('semester_plan.id as content_planid,content,content_start_date,content_end_date,content_completed_date,content_plan_status');
     ...
     public function semester_plan_chapter_edit($semester,$subject,$teacher){
        ...
        $this->db->where("(chapter_plan_status is NULL OR chapter_plan_status = 0)");
        $this->db->where('data_type',2);
        $this->db->where('data_parent',$subject);
        $this->db->where("(semester ='$semester' OR semester is NULL)");
        $this->db->where("(subject ='$subject' OR subject is NULL)");
     ...
     public function semester_plan_content_edit($chapter,$id,$semester){
        
        $batch = $this->Api_student->get_semester_batch($semester);
       $this->db->select('semester_plan.id as content_planid,chapter_design.id as content_id, chapter_design.chapter_id, `type`, `type_id`, `status`, `topic_title`,content_start_date,content_end_date,content_completed_date,content_plan_status');
        $this->db->where('chapter_design.chapter_id',$chapter);
        $this->db->where('chapter_design.status >',0);
        $this->db->where("( semester_plan.content_plan_status = 0  OR semester_plan.content_plan_status is NULL)");
    ...
    public function get_wall($batch,$limit,$offset,$uid){
        ...
        foreach ($wall as $key => $value) {
            ...
            if($value['semester'] ==0){
                $sm =  $this->get_semester_subjectbatch($value['batch_id'],$value['subject_id']);
                if($sm != ""){
                    $wall[$key]['semester'] = $sm;
                }
            }

            $batchdet = $this->get_batch($value['batch_id']);
            $wall[$key]['batch_name'] = $batchdet['batch_name'];
        ...
        foreach ($wall as $key => $value) {
            ...
            if($value['type']=='content' && $value['content_type'] =="presentation"  && $value['image'] !=""){
                ...
                // }else{
                    $prs_details = $this->Api_student->get_presentation_byid($value['type_id']);
                    $tpc = 0;
                    if($prs_details['copy_topic'] != 0 ){
                        $tpc = $prs_details['copy_topic'];
                    }else if($value['type_id'] != 0){
                        $tpc = $value['type_id'];
                    }
...
// print_r( $batchcode);
    ...
    public function get_doubt_teacher($teacher){
        $this->db->select('message as body,date,notifications_user.is_read as read_status,type,content_id,student.name,topic_id,presentation_qanda_questions.qanda_id');
        // $this->db->where('created_by',$teacher);
        $this->db->where('user_type','teacher');
        $this->db->where('user_id',$teacher);
        // $this->db->where('data_status',1);
        ...
        foreach ($result as $key => $value) {
            if(($value['type'] == 'content_publish') || ($value['type'] == 'chapter_publish')){
                ...
                $semid =  $sem_result['sem_id'];
                 ...
                 $topic = $this->get_cloned_parentid($value['topic_id'],$teacher);
                 $slide = $this->get_topic_main_parent($topic);
                 $thumb_nail = base_url().'uploads/slide/img'.$topic.".jpg";
                 $result[$key]['thumbnail'] = $thumb_nail;

                 $question_id = $value['qanda_id'];
                 $answersd = $this->Api_student->get_topic_qanda_answer($value['topic_id'],$question_id);
                 // print_r($answers);
                 foreach ($answersd as $key1 => $value1) {
                    $result[$key]['answer'] = $value1;
                     break;
                 }
    ...
    public function get_wall_message_teacher($limit,$offset,$uid){
        ...
        foreach ($wall as $key => $value) {
            ...
            $userlike = $this->userlike($value['wall_id'],$uid,'teacher');
            if($value['image']){
	            $wall[$key]['image'] = base_url().'uploads/wall/'.$value['image'];
    ...
    // public function get_batch_semester_livestatus($batch){
        

    //     // $status =  array(1,2);
    //     $this->db->select('sem_id as semester_id,sem_name as semester_name');
    //     $this->db->where('ins_semester.batch_id',$batch);
    //     $this->db->where('ins_semester.publish_status',);
    //     $this->db->from('ins_semester');
    //     $query = $this->db->get();
    //     $result = $query->row_array();
    ...
    public function teacher_batch_list($id){
        ...
        foreach ($result as $key => $value) {
           	$result[$key]['invite_student_count'] = $this->Api_student->get_invite_student_count($value['batch_id']);
            $studcount = $this->Api_student->get_student_count($value['batch_id']);
            $result[$key]['student_count'] =$studcount;
...
public function get_semester_subjectbatch($batch,$subject){
    ...
    public function contents_total($semester,$subject){
        // $this->db->select('');
        $this->db->where('semester_plan.semester',$semester);
        $this->db->where('subject',$subject);
        $this->db->where("(content_plan_status =1 OR content_plan_status = 2)");
        $this->db->from('semester_plan');
        $this->db->join('chapter_plan','chapter_plan.id = semester_plan.chapter_plan_id','left');
        $query = $this->db->get();
        $result = $query->num_rows();
        return $result;
    }
    ...
    public function contents_current($semester,$subject){
        // $this->db->select('');
        $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function contents_current_completed($semester,$subject){
        // $this->db->select('');
         $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function monthly_contents_total($semester,$month,$subject){
        // $this->db->select('');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function monthly_contents_completed($semester,$month,$subject){
        // $this->db->select('');
         $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
     ...
     public function contents_to_complete($semester,$subject){
        // $this->db->select('');
        $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function contents_to_complete_month($semester,$month,$subject){
        // $this->db->select('');
        $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function contents_completed($semester,$subject){
        // $this->db->select('');
         $date = date('Y-m-d');
        $this->db->where('semester_plan.semester',$semester);
    ...
    public function get_chapter_status($chapter){
     ...
     //current app start -  with chapter content partid as content id/////////////////////
    public function get_chapter_part_new($part,$id,$batch){
        $this->db->select('chapter_design.id as part_id,type,type_id,chapter_content_publish.teacher_id,chapter_design.chapter_id,chapter_design.image,chapter_design.topic_title');
        $this->db->where('chapter_design.id',$part);
        $this->db->where('chapter_design.status',1);
...
// }
    ...
    // current app close/////////////////////
         ...
         public function get_assignment_view_teacher($asid){
        $this->db->where('assignment_id',$asid);
        $this->db->where('assignment_status >',0);
        $result  =  $this->db->get('assignment')->row_array();

                $assignment_student_view_percentage = $this->student_assignment_view_percentage($asid);
                $result['percentage'] = $assignment_student_view_percentage;
                $assignment_files_question = array();
                $assignment_files_q = $this->assignment_files_question($asid);
                foreach ($assignment_files_q as $key => $value) {
                    $assignment_files_question[] = base_url().'uploads/assignment_question/'.$value['file'];
                ...
                $result['assignment_files_question'] = $assignment_files_question;
                    ...
                    $result['students'] = $this->assignment_students($asid);
    ...
    public function assignment_students($assignment_id){
        ...
        $this->db->where('status',1);
        $this->db->where('assignment_id',$assignment_id);
        $this->db->join('assignment','assignment.batch_id=student.batch_id','left');
        ...
        $query = $this->db->get();
         ...
         foreach ($result as $key => $value) {
            ...
            $mark = $this->check_student_marks($value['student_id'],$assignment_id);
            if(!empty($view)){
                $score = $mark['marks'] ;
            }else{
                $score = 0;
            ...
            $this->db->select('*');
            $this->db->where('user_id',$value['student_id']);
            $this->db->where('assignment_id',$assignment_id);
            $this->db->where('status',1);
            $submit_assignment = $this->db->get('assignment_submit')->num_rows();

            if($submit_assignment ==0 && $score == 0){
                $view = 0;
            }
            else if($submit_assignment !=0 && $score == 0){
                $view = 1;
            }else if($submit_assignment !=0 && $score != 0){
                $view = 2;
            }
