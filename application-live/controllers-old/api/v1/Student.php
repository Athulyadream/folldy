               ...
               if($check['status']== 0){
                ...
                // if($check['status']== 0){
                    ...
                    // echo $uid." - ".$type;
                        ...
                        $assignment['pending'] = $this->Api_student->list_assignment_teacher($id,$batch,$subject_id);
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    foreach ($week as $key => $value) {
                        ...
                        if(!empty($data)){
                            foreach($data as $row){
                               $days[$value['week_days']][]= $row['batch_name'];
                            }
                        }
                        else{
                            $days[$value['week_days']] ='';
...
public function plan_teacher_post(){
         $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
            ...
            // print_r($check);
               if($check['status']== 0){
                ...
                // if($check['status']== 0){
                    ...
                    $contentsplan['months'] = $months_name;

                    $contentstotal = $this->Api_student->contents_total($semester);
                    $contents_current = $this->Api_student->contents_current($semester);
                    $contents_current_completed = $this->Api_student->contents_current_completed($semester);
                    $contents_current_prcentage = 0;
                    $contents_current_completed_prcentage = 0;

                    if($contentstotal){
                        $contents_current_prce = (int)$contents_current/(int)$contentstotal;
                        $contents_current_prcentage = (int)$contents_current_prce*100 ." %";
                   
                        $contents_current_completed_prce = (int)$contents_current_completed/(int)$contentstotal;
                        $contents_current_completed_prcentage = (int)$contents_current_completed_prce*100 ." %";
                    }
                    // print_r($months);
                    // die();
                    $contentsplan['current'] = $contents_current_prcentage;
                    $contentsplan['current_completed'] = $contents_current_completed_prcentage;
                    ...
                    foreach ($months as $key => $value) {
                        ...
                        $monthly_contentstotal = $this->Api_student->monthly_contents_total($semester,$value);
                        $monthly_contents = $this->Api_student->contents_to_complete($semester,$value);
                        $monthly_contents_completed = $this->Api_student->monthly_contents_completed($semester,$value);
                        $monthly_contents_prcentage = 0;
                        $monthly_contents_completed_prcentage = 0;

                        if($contentstotal){
                            $monthly_contents_prce = (int)$monthly_contents/(int)$contentstotal;
                            $monthly_contents_prcentage = (int)$monthly_contents_prce*100 ." %";

                            $monthly_contents_completed_prce = (int)$monthly_contents_completed/(int)$contentstotal;
                            $monthly_contents_completed_prcentage = (int)$monthly_contents_completed_prce*100 ." %";
                        }
                        $monthlyPlan[$months_name[$key]]['completed'] = $monthly_contents_completed_prcentage;
                        $monthlyPlan[$months_name[$key]]['notcompleted'] = $monthly_contents_prcentage;
                    ...
                    $plan['monthlycontents'] = $monthlyPlan;
                    $plan['totalcontent'] = $contentstotal;
                    $plan['contents_to_complete'] = $this->Api_student->contents_to_complete($semester);
                    $plan['completed'] = $this->Api_student->contents_completed($semester);
                    $percentage = 0;
                    if($contentstotal){
                        $contents_completed_prce = (int)$contents_completed/(int)$contentstotal;
                        $percentage = (int)$contents_completed_prce*100 ." %";
                    }
