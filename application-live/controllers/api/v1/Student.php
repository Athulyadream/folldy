...
//////////////////***************** New updations-  athulya************///////////
            //*****************Recent Presentation************//
                    ...
                    if($semester){
                        ...
                        $semesterb = $this->Api_student->get_active_semester($student['batch_id']);
                         // $semesterb =  $semesters['semester_id']
                    }
            $recentpresentation = $this->Api_student->get_recentpresentations($id,$semesterb);
                        //*****************Recent Chapter************//
            $recentchapter = $this->Api_student->get_recentchapters($id,$semesterb);
        //////////////////////////close Athulya/////////////////////////////////
             ////////////////////////new update - bhagya ///////////////////
                 if(empty($semester)){
            ...
            if($id){
                ...
                if($c== 0){
                    ...
                    foreach ($chapter as $key => $value) {
                        $part=array();
                       $part = $this->Api_student->get_chapter_details_library_new($value['chapter_id'],$teacher);
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    //     foreach ($teachers as $key => $value) {
                        ...
                        $tid = 0;
                            $chapter1 = $this->Api_student->get_chapter_list_in($subject,$id,$batch);
...
$pname ="";
                            if($type =='1'){
                                ...
                                //presentation
                                $typ='presentation';
                                $prs_details = $this->Api_student->get_presentation_byid($type_id);
...
public function list_assignment_teacher_post(){
         $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
               ...
               if($check['status']== 0){
                ...
                // if($check['status']== 0){
                    ...
                    // echo $uid." - ".$type;
                        ...
                        $assignment = $this->Api_student->list_assignment_teacher($id,$batch,$subject_id);
                        // $assignment['closed'] = $this->Api_student->list_closed_assignment_teacher($id,$batch,$subject_id);   
             ...
             $check = $this->Api_student->check_session_teacher($id,$session);
                if($check['status']== 0){
                    $out = array('status'=>3,'message'=>'Session does not match.');
                     ...
                     // $upload_data = $this->upload->data();
$semester = $this->Api_student->get_semester_subjectbatch($batch,$subject);
...
// upload file only for add assignment
    ...
    public function upload_assignment_question_post(){
                ...
                if($check['status']== 0){
                    ...
                    //echo $teacher;
                            ...
                            if ( ! $this->upload->do_upload('file'))
                            {
                                ...
                                $upload_data = $this->upload->data();
                                $filename = base_url().'uploads/assignment_question/'.$upload_data['file_name'];
                            }


                    if($filename){ 
                        $out = array('status'=>1,'message'=>'file added','data'=>$filename);
                           $this->Api_student->insert_log("APP - file submit ",$id,'Teacher' );
                       
                            
                    }
                    else{
                        $out = array('status'=>0,'message'=>'file Add Failed','data'=>new stdClass);
                    } 
                }
            }
            elseif(!$id){
      ...
      public function upload_assignment_audio_post(){
         $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
             ...
             $check = $this->Api_student->check_session_teacher($id,$session);
                if($check['status']== 0){
                    ...
                    //echo $teacher;
                                ...
                                $ext = pathinfo($_FILES['publish_audio']['name'], PATHINFO_EXTENSION);
                                // change name
                                $audioname = $file_name . time() . "." . $ext;  
                                $target_file = './uploads/wall/'.$audioname;

                                if (move_uploaded_file($_FILES["publish_audio"]["tmp_name"], $target_file)) {
                                    $filename = base_url().'/uploads/wall/'.$audioname;
                                    $out = array('status'=>1,'message'=>'file added','data'=>$filename);
                                    $this->Api_student->insert_log("APP - file submit ",$id,'Teacher' );
                                }else{
                                    $out = array('status'=>0,'message'=>'file Add Failed','data'=>new stdClass);
...
// submit assignment file- upload only file for submit assignment
    ...
    public function publish_marks_post(){
        ...
        $value =!empty($input['status']) ? $input['status'] : '';
        $status = $this->post('status') ? $this->post('status') : $value;
            ...
            if($id && $assignment_id){
             $check = $this->Api_student->check_session_teacher($id,$session);
                if($check['status']== 0){
                    ...
                    $data = array(
                        'assignment_publish_status'=>$status,
                        'updated_on'=>date("Y-m-d"),
                        'updated_by'=>$id
                    );
                if($this->Api_student->edit_assignment($assignment_id,$data)){
                        if($status == 1){
                           ...
                           $this->Api_student->insert_log("APP - Assignment marks published id".$assignment_id,$id,'teacher' );
                       	 ...
                       	 $out = array('status'=>1,'message'=>'Assignment marks unpublished');
                           $this->Api_student->insert_log("APP - Assignment marks unpublished id".$assignment_id,$id,'teacher' );
                       }
...
////////////////////////////////////////semester plan/////////////////////////////////////////////
    ...
    public function chapter_plan_submit_listing_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
            ...
            if($id  && $semester && $subject){
                ...
                if($check['status']== 0){
                    ...
                    if($plan){
                            ...
                            $out = array('status'=>0,'message'=>'There is no content','data'=>new stdClass);
                        } 
                    }
            }
            elseif(!$id){
     ...
     public function plan_content_submit_listing_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $chapter && $semester && $subject){
                $check = $this->Api_student->check_session_teacher($id,$session);
                if($check['status']== 0){
                    $out = array('status'=>3,'message'=>'Session does not match.');
                }
    ...
    public function complete_plan_content_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        $value =!empty($input['contentplan_id']) ? $input['contentplan_id'] : '';
        $contentplan_id = $this->post('contentplan_id') ? $this->post('contentplan_id') : $value;
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $contentplan_id){
                ...
                if($check['status']== 0){
                    ...
                    $data = array(
                        'content_plan_status' =>2,
                        'content_completed_date' => date('Y-m-d')
                    );
                    $content_id = $this->Api_student->update_expiry_date_content($data,$contentplan_id);
                    if($content_id){
                            $out = array('status'=>1,'message'=>'content completed','data'=>$content_id);
                            $this->Api_student->insert_log("content completed ".$contentplan_id,$id,'teacher' );
                        }
                        else{
                            $out = array('status'=>0,'message'=>'complete plan failed','data'=>new stdClass);
                        } 
                    }
            }
            elseif(!$id){
                ...
                $null = new stdClass;
                $out = array('status'=>0,'message'=>'teacher id field is mandatory','data'=>$null);
            }elseif(!$contentplan_id){
         
                $null = new stdClass;
                $out = array('status'=>0,'message'=>'content plan id field is mandatory','data'=>$null);
            }
        }
        $this->response($out);
     ...
     public function submit_content_plan_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        $chapter_plan_id = $this->post('chapter_plan_id') ? $this->post('chapter_plan_id') : $value;
       

        $value =!empty($input['data']) ? $input['data'] : '';
        $data = $this->post('data') ? $this->post('data') : $value;

        // $value =!empty($input['content']) ? $input['content'] : '';
        // $content = $this->post('content') ? $this->post('content') : $value;

       
        // $value =!empty($input['startdate']) ? $input['startdate'] : '';
        // $startdate = $this->post('startdate') ? $this->post('startdate') : $value;

        // $value =!empty($input['enddate']) ? $input['enddate'] : '';
        // $enddate = $this->post('enddate') ? $this->post('enddate') : $value;
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $data){
                ...
                if($check['status']== 0){
                    ...
                    $where['semester'] = $semester;
                    $where['chapter_plan_id'] = $chapter_plan_id;
                    // $where['teacher'] = $id;
                    $where['chapter'] = $chapter;
                    $where['content_plan_status'] = 0;


                    // check exist not approved data
                    $check_expiry = $this->Api_student->check_content_count($where);
                    if($check_expiry == 0){
                        $insert = array(
                                'semester' => $semester,
                                'chapter' => $chapter,
                        ...
                        foreach ($data as $key => $value) {
                            $insert['content_id'] = $value['content_id']; //existing content
                            $insert['content'] = $value['content']; // for new content
                            $insert['content_start_date'] = $value['content_start_date'];
                            $insert['content_end_date'] = $value['content_end_date'];
                            $result=$this->Api_student->add_data($insert,'semester_plan');
                        }

                       
                       
                        $out = array('status'=>1,'message'=>'','data'=>$result);
                        $this->Api_student->insert_log("Contetnt Plan added ".$chapter,$id,'teacher' );
                    }
                    else{
                        ...
                        foreach ($data as $key => $value) {
                            $chdata['content_id'] = $value['content_id']; //existing content
                            $chdata['content'] = $value['content']; // for new content
                            $update['content_id'] = $value['content_id']; //existing content
                            $update['content'] = $value['content']; // for new content
                            $update['content_start_date'] = $value['content_start_date'];
                            $update['content_end_date'] = $value['content_end_date'];
                            $check_content = $this->Api_student->check_content_count($chdata);
                            $content_plan_id = $check_content['id'];
                            $result=$this->Api_student->update_expiry_date_content($update,$content_plan_id);
                        }

                       
                        $out = array('status'=>1,'message'=>'Update content plan','data'=>'');
            ...
            elseif(!$id){
                ...
                $null = new stdClass;
                $out = array('status'=>0,'message'=>'data  field is mandatory','data'=>$null);
            }
        }
        $this->response($out);
    ...
    public function submit_chapter_plan_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $subject){
                ...
                if($check['status']== 0){
                    ...
                    // check exist not approved data
                    $check_expiry = $this->Api_student->check_chapter_count($where);
                    if($check_expiry == 0){
                        $insert = array(
                                'semester' => $semester,
                                'subject' => $subject,
                        ...
                        foreach ($data as $key => $value) {
                            ...
                            $insert['chapter'] = $value['chapter']; // for new content
                            $insert['start_date'] = $value['start_date'];
                            $insert['end_date'] = $value['end_date'];
                        ...
                        foreach ($data as $key => $value) {
                            $chdata['chapter'] = $value['chapter']; //existing content
                            $update['chapter'] = $value['chapter']; //existing content
                            $update['start_date'] = $value['start_date'];
                            $update['end_date'] = $value['end_date'];
                            $check_content = $this->Api_student->check_expiry_date($chdata);
                            $content_plan_id = $check_content['id'];
                            $result=$this->Api_student->update_expiry_date($update,$content_plan_id);
                        }
...
// add more content name only
     public function add_contentplan_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $content){
                ...
                if($check['status']== 0){
                    ...
                    $check_expiry = $this->Api_student->check_exiry_date_content($where);
                    if(empty($check_expiry)){
                        $insert = array(
                                'semester' => $semester,
                                'chapter' => $chapter,
                        ...
                        $result=$this->Api_student->add_data($insert,'semester_plan');
                          $out = array('status'=>1,'message'=>'','data'=>$result);
                            $this->Api_student->insert_log("Contetnt Plan added ".$result,$id,'teacher' );
                    }
                    else{
                        $out = array('status'=>1,'message'=>'Add content plan failed','data'=>'');
...
public function chapter_plan_listing_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id  && $semester && $subject){
                ...
                if($check['status']== 0){
                    ...
                    // }
                    $whered['semester'] = $semester;
                    $whered['subject'] = $subject;
                    $whered['chapter_plan_status'] = 0;

                    $check_expiry = $this->Api_student->check_chapter_count($whered);
                    if($check_expiry !=0){
                    $plan = $this->Api_student->semester_plan_chapter_edit($semester,$subject,$id);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $chapter && $semester && $subject){
                ...
                if($check['status']== 0){
                    ...
                    $check_expiry = $this->Api_student->check_content_count($where);
                    if($check_expiry !=0){
                        $plan = $this->Api_student->semester_plan_content_edit($chapter,$id,$semester);

                    }else{
                        $plan = $this->Api_student->semester_plan_content($chapter,$id,$semester);

                    }
                    // $plan = $this->Api_student->semester_plan_content();
                    if($plan){
                            $out = array('status'=>1,'message'=>'','data'=>$plan);
                            $this->Api_student->insert_log("APP -semesterlisting ".$chapter,$id,'teacher' );
                        }
                        else{
                            $out = array('status'=>0,'message'=>'chapterlisting failed','data'=>new stdClass);
                        } 
...
//////////////////////////////////////Add new wall///////////////////////////////////////////////////
    ...
    public function add_wall_post(){
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
                    ...
                    $out = array('status'=>3,'message'=>'Session does not match.');
                             ...
                             if(isset($_FILES['publish_audio'])){
                                ...
                                if (move_uploaded_file($_FILES["publish_audio"]["tmp_name"], $target_file)) {
                                 ...
                                 $out = array('status'=>0,'message'=>'audio upload failed','data'=>new stdClass);
                                }
                            }
                                    ...
                                    if ( ! $this->upload->do_upload('image'))
                                    {
                                        ...
                                        $datan['image'] = $upload_data_image['file_name'];
                                    }

                                    $wall_id = $this->Api_student->add_data($datan,'wall_notifications');
                                      
                                        if($wall_id){ 
                                            $out = array('status'=>1,'message'=>'wall post added','data'=>$wall_id);
                                               $this->Api_student->insert_log("APP - wall post added".$wall_id,$id,'student' );
                                        }
                                        else{
            ...
            $null = new stdClass;
            $out = array('status'=>0,'message'=>'message field is mandatory','data'=>$null);
            }
        }
        $this->response($out);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $offset = $offset-1; 
                    $offset = ($offset*10);
                    $wall = $this->Api_student->get_wall_teacher($limit,$offset,$id);
                    if(!empty($wall)){ 
                        $out = array('status'=>1,'message'=>'','data'=>$wall);
                    }
                    else{
...
// die();
            ...
            ///////////////////////////////////////////////////////////////////////
                    ...
                    if(!empty($student)){ 
                        ...
                        $timetable[] = array('name'=>'Monday','subject'=> $days['Monday'] ? $days['Monday'] : array());
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($c== 0){
                    ...
                    $batch_details = $this->Api_student->get_batch($batch);
                    $batch_name = $batch_details['batch_name'];

                    $submitedpercentage =$this->Api_student->submited_assignments_percentage($id);
                    $count = $submitedpercentage.'% Submit'; // total submited assignment percentage
                    $count1 = '0% Learned'; // total learned percentage
                    $count2 = "Batch ".$batch_name; //current batch
...
function time_table_new_post(){
    ...
    function teacher_mainpage_post(){
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $profile = $this->Api_student->getProfileDetails_teacher($id);
                    $timetable = $this->Api_student->today_timetable_teacher($id);
                    ...
                    foreach ($week as $key => $value) {
                        $data ='';
                        $data = $this->Api_student->get_timetable_teachers($id,$value['id']);
                        if(!empty($data)){
                            foreach($data as $row){
                                $arrayName = array('batch' => $row['batch_name'],'subjectname'=> $row['subject']);
                               $days[$value['week_days']][]= $arrayName ;


                            }
                        }
                        else{
                            $days[$value['week_days']]  = array();
                        }
                    ...
                    if(!empty($profile)){ 
                        ...
                        $dataT = array();
                    $dataT['school_name'] = $profile['institute_name'];
                    $dataT['school_name_last '] = $profile['abbreviation'];
                    ...
                    $profiles['email'] = $profile['teacher_email'];
                    $profiles['phone'] = $profile['teacher_phone'];
                    $dataT['profile'] = $profiles;

                   
                    $dataT['notification_count'] = $this->Api_student->get_notifications_teacher_count($id);
                  
                    $dataT['timetable'] = $timetable;
                    $dataT['doubt'] = $this->Api_student->get_doubt_teacher_count($id);
                    $dataT['pending'] = $this->Api_student->get_count_invite_students($id);
                    ...
                    if(!empty($profile)){ 
                        $out = array('status'=>1,'message'=>'','data'=>$dataT);
                    }
                    else{
                        $out = array('status'=>0,'message'=>'There is no teacher','data'=>new stdClass);
                    }
    ...
    function doubt_list_post(){
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $doubt = $this->Api_student->get_doubt_teacher($id);

                    // $nocount = $this->Api_student->get_notifications_student_count($id);
                    // $semester = $this->Api_student->get_semester($student['batch_id']);

                    if(!empty($doubt)){ 
                        $out = array('status'=>1,'message'=>'','data'=>$doubt);
                    }
                    else{
                        $out = array('status'=>0,'message'=>'There is no teacher','data'=>new stdClass);
                    }
     ...
     function wall_messages_list_post(){
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                   ...
                   $limit = 0;
                   $offset = 0;
                   $doubt = $this->Api_student->get_wall_message_teacher($limit,$offset,$id);
    ...
    function delete_wall_messages_post(){
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                   ...
                   $doubt = $this->Api_student->delete_wall($data);

                    // $nocount = $this->Api_student->get_notifications_student_count($id);
                    // $semester = $this->Api_student->get_semester($student['batch_id']);

                    if($doubt){ 
                        $out = array('status'=>1,'message'=>'Message deleted successfully');
                    }
                    else{
                        $out = array('status'=>0,'message'=>'There is no data','data'=>new stdClass);
                    }
    ...
    function batch_list_teacher_post(){
        ...
        if (!$this->post('userid') && json_last_error() != 0) {
            ...
            if($id){
                ...
                if($check['status']== 0){
                    ...
                    $batch = $this->Api_student->teacher_batch_list($id);
                    ...
                    if(!empty($batch)){ 
                        ...
                        $out = array('status'=>0,'message'=>'There is no batch','data'=>new stdClass);
                    }
...
//////////////////////////close Athulya/////////////////////////////////
    ...
    public function view_assignment_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
            ...
            if($id && $assignment_id){
             $check = $this->Api_student->check_session($id,$session);
                if($check['status']== 0){
                   ...
                   $check_data = $this->Api_student->check_assignment_view($id,$assignment_id);
                   if(empty($check_data)){
                         $data = array(
                            'assignment_id'=>$assignment_id,
                            'student_id'=>$id,
                        ...
                        if($this->Api_student->add_data($data,'assignment_student_view')){
                            $out = array('status'=>1,'message'=>'Assignment view added');
                            $this->Api_student->insert_log("APP - Assignment  view added".$assignment_id,$id,'student' );
                        }
                        else{
                            $out = array('status'=>0,'message'=>'Assignment view failed','data'=>new stdClass);
                        } 
                   }else{
                        $out = array('status'=>0,'message'=>'Assignment already viewed','data'=>new stdClass);
...
///analysis
public function plan_status_teacher_post(){
         $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
            ...
            // print_r($check);
               if($check['status']== 0){
                ...
                // if($check['status']== 0){
                    ...
                    $semester =  $this->Api_student->get_semester_subjectbatch($batch,$subject_id);
                 
                    $sem_details = $this->Api_student->get_semester_details($semester);

                    $startdate = $sem_details['semester_start_date'];
                    $enddate = $sem_details['semester_end_date'];
   // print_r($startdate.$enddate);
   //                  die();

                    $months_name = $this->get_months_name($startdate,$enddate);
                    $months_name  = array_unique($months_name);
                    $contentsplan['months'] = array_values($months_name);

                    $contentstotal = $this->Api_student->contents_total($semester,$subject_id);
                    $contents_current = $this->Api_student->contents_current($semester,$subject_id);
                    // print_r($contents_current);
                    // die();
                    $contents_current_completed = $this->Api_student->contents_current_completed($semester,$subject_id);
                    // print_r($contents_current_completed);
                    // die();
                    $contents_current_prcentage = 0;
                    $contents_current_completed_prcentage = 0;

                    if($contentstotal){
                        $contents_current_prce = (float)$contents_current/(float)$contentstotal;
                        $contents_current_prcentage = (float)$contents_current_prce*100;
                   // print_r($contentstotal);
                   //  die();
                        $contents_current_completed_prce = (float)$contents_current_completed/(float)$contentstotal;
                        $contents_current_completed_prcentage = (float)$contents_current_completed_prce*100;
                        // print_r($contents_current_completed_prcentage);
                        // die();
                    }
                    // print_r($months);
                    // die();
                    $contentsplan['plan_percentage'] = round($contents_current_prcentage,2);
                    $contentsplan['completed_percentage'] = round($contents_current_completed_prcentage,2);
                    ...
                    foreach ($months as $key => $value) {
                        ...
                        $nmonth = date("m", strtotime($value));
                        $monthly_contentstotal = $this->Api_student->monthly_contents_total($semester,$nmonth,$subject_id);
                        // print_r($monthly_contentstotal); echo "<br/>";
                        $monthly_contents = $this->Api_student->contents_to_complete_month($semester,$nmonth,$subject_id);
                        // print_r($monthly_contents);

                        $monthly_contents_completed = $this->Api_student->monthly_contents_completed($semester,$nmonth,$subject_id);
                        $monthly_contents_prcentage = 0;
                        $monthly_contents_completed_prcentage = 0;

                        if($contentstotal){
                            $monthly_contents_prce = (float)$monthly_contents/(float)$contentstotal;
                            $monthly_contents_prcentage = (float)$monthly_contents_prce*100;

                            $monthly_contents_completed_prce = (float)$monthly_contents_completed/(float)$contentstotal;
                            $monthly_contents_completed_prcentage = (float)$monthly_contents_completed_prce*100;
                        }
                        $monthlyPlan[$key]['name'] = $months_name[$key];
                        $completed_mon = round($monthly_contents_completed_prcentage,2);
                        $notcompleted_mon = round($monthly_contents_prcentage,2);



                        $monthlyPlan[$key]['completed'] = $completed_mon;
                        $monthlyPlan[$key]['notcompleted'] = $notcompleted_mon;
                        $grandtotal = (float)$completed_mon+(float)$notcompleted_mon;
                        if($grandtotal !=0)
                        {
                            $grand_prce = (float)$completed_mon/(float)$grandtotal;
                            $grand_percentage =  (float)$grand_prce*100;
                        }else{
                            $grand_percentage = 0;
                        }
                        ...
                        // print_r($grand_prce);
                        // echo "<br>";
                        $monthlyPlan[$key]['monthlycompleted'] =round($grand_percentage,2);
                    ...
                    $plan['contents'] = $contentsplan;
                    $plan['monthlycontents'] = array_values($monthlyPlan);
                    $plan['totalcontent'] = $contentstotal;
                    $plan['contents_to_complete'] = $this->Api_student->contents_to_complete($semester,$subject_id);
                    $contents_completed = $this->Api_student->contents_completed($semester,$subject_id);

                    $plan['completed'] = $contents_completed;
                    $percentage = 0;
                    if($contentstotal !=0){
                        $contents_completed_prce = (float)$contents_completed/(float)$contentstotal;
                        $percentage = (float)$contents_completed_prce*100 ;
                        // print_r($contents_completed);
                        // die();
                    }
                   
                    $plan['percentage'] = round($percentage,2);
    ...
    function get_months_name($date1, $date2) {
    ...
    function chapter_content_new_post(){
            ...
            if($id){
                ...
                if($c== 0){
                    $out = array('status'=>3,'message'=>'Session does not match.');
                      ...
                      $batch = $this->Api_student->get_semester_batch($semester);
                    $data = $this->Api_student->get_chapter_part_new($part_id,$id,$batch);
//                     if($id == '35'){
//     print_r($data['type']);
//     die();
// }
    ...
    ///type id current app
    function chapter_content_presentation_post(){
                ...
                if($c== 0){
                    ...
                    foreach ($chapter as $key => $value) {
                       ...
                       foreach ($part as $key1 => $value1) {
                          ...
                          elseif($value1['type']==2){
                            $detail[$key1]['part_id'] =$value1['type_id'];
                            $detail[$key1]['part_name'] =$value1['topic_title'];
                            $detail[$key1]['type'] ='presentaion';
                          }
                          elseif($value1['type']==4){
...
public function view_assignment_teacher_details_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $assignment_id){
             $check = $this->Api_student->check_session_teacher($id,$session);
                ...
                $data_ass = $this->Api_student->get_assignment_view_teacher($assignment_id,$id);
                // print_r($data_ass);
                // die();
                // $subject = $data_ass['subject'];
                // $semester = $data_ass['semester'];
    ...
    public function view_assignment_students_teacher_post(){
        $this->benchmark->mark('start');
        $json = file_get_contents('php://input');
        $input = json_decode($json,true);
        ...
        if (!$this->post('user_id') && json_last_error() != 0) {
            ...
            if($id && $assignment_id){
             $check = $this->Api_student->check_session_teacher($id,$session);
                ...
                $data_ass = $this->Api_student->get_assignment_view_student_teacher($assignment_id,$student_id);
                // print_r($data_ass);
                // die();
                // $subject = $data_ass['subject'];
                // $semester = $data_ass['semester'];
    ...
    //                         $res1 = $this->fcm->send_push_message_not('', $var, $data2);
    // }
