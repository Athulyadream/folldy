...
class PresentationController extends CI_Controller {
	...
	function __construct(){
     ...
     //    if($this->session->userdata('admin_id') == "" && $this->session->userdata('teacher_id') != ""){
    	...
    	if(($this->uri->segment(1) != 'presentation-webview') AND ($this->uri->segment(1) != 'chapter-webview')){
	        if((!$this->session->userdata('admin_id'))&&(!$this->session->userdata('teacher_id'))){
	            redirect(base_url());
	        }
	        if($this->session->userdata('admin_id') == "" && $this->session->userdata('teacher_id') != ""){
...
public function get_slide_search(){
	...
	public function chapter_webview($cid){
        ...
        //    		$body_data = array(
           		...
           		if($c == 1){
           			$uid = $user_id;
           			$content = $this->PresentationModel->chapter_teachercontent_webview($cid,$uid);

           		}else if($c == 2){
           			 $uid = $this->Api_student->get_subject_teacher($subject,$user_id); 

           			 if($uid){
           			 	$content = $this->PresentationModel->chapter_content_webview($cid,$uid);
