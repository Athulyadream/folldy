...
/************************25/02/2021 start athul sasidaran************************************/
	...
	public function dashboard_chapter1(){
		...
		$sem = $this->TeacherModel->get_semester_subjectbatch($batchid,$id);
		$this->session->set_userdata('sem_id',$sem);
// 		if($this->session->userdata('teacher_id') == '35'){
// 			echo "".$batchid.",".$id."<br/>";
// 			print_r($sem);
// 			echo "<br/>";
// 			echo $this->session->userdata('sem_id')."seme ";
// 			echo "ddddddd<br/>";
// die();
// 		}
		$body_data = array(
...
////////////************************************************************************/////////////////
	...
	public function add_assignment(){
		...
		$question = trim($this->input->post('assignment_question'));
			...
			if(!empty($_FILES['files']['name']) && count(array_filter($_FILES['files']['name'])) > 0){ 
			  ...
			  $subjectname = $this->Api_student->get_subject_data($subject);
                            $batch = $this->Api_student->get_semester_batch($semester);
                                ...
                                $teachern = $this->Api_student->getProfileDetails_teacher($this->session->userdata('teacher_id'));
                                $teachername = $teachern['teacher_name'];
                                $datan = array(
                                        'batch_id' => $batch,
                                        'title'=> $title,
