...
// alert(nextIndex);
    ...
    var dataid = $('.current').attr('data-topic');
    if ( $(".last-child").parents(".current").length == 1 && $(".hover-item").parents(".current").length == 0 ) { 
        var did = $('.current').attr('data-parent');
         var section = $("."+did).attr('id');
         goToSlide(section);
       
    }else{
        if (dataid != 'undefined') {
          $('.viewtop'+ dataid +'').each(function() {
                var element = $(this);
                // if(!element.hasClass('viewed')){
                  // alert(".viewtop"+ dataid);
                      element.removeClass( "viewtop"+ dataid );
